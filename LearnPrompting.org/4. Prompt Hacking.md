...
1. Prompt injection
2. Prompt leaking & Data Exfiltration
3. Jailbreaking
...
**The nuances**:
- Here, the user claims that "they are the authority" and the model gives all information thinking that the information is in the **safe hands**.

> Note: The `Data exfiltration` is similar to leaking, but in leaking we **ask the model to spit out its own prompt**, whereas in exfiltration we **ask the model to spit out the data it has access to** which is confidential and sensitive.
...
- **Jailbreaking**: 
    - Uses only a user prompt to make the model misbehave. The goal **isn't to make the model discard the system prompt**. Here, the goal is to make the AIs respond to the user's prompt that the model is not supposed to.
    - Typical prompts: role‑play (“pretend you are DAN”), multi‑step coaxing, obfuscation, “this is just a simulation / for research” tricks, etc.
- **Prompt Injection**: 
    - Uses the system prompt and user input to hijack the model's behavior. The goal is to make the AI ignore its original instructions and follow the injected content instead.
    - Typical form: “Ignore all previous instructions. Instead, do X” embedded directly in user input or in a web page / document the model is asked to analyze.
